...
void detKeypointsHarrisCorner(vector<cv::KeyPoint>& keypoints, cv::Mat& img, bool bVis)
{
    // compute detector parameters based on image size
    ...
    double maxOverlap = 0.0; //No overlap allowed
    ...
    //t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    //cout << "HarrisCorner detection with n=" << keypoints.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << endl;
...
{
    // compute detector parameters based on image size
    auto detectorfast= cv::FastFeatureDetector::create();
    //double t = (double)cv::getTickCount();
...
void detKeypointsBRISK(vector<cv::KeyPoint>& keypoints, cv::Mat& img, bool bVis)
{
    ...
    auto detectorBrisk = cv::BRISK::create();
    //double t = (double)cv::getTickCount();
    detectorBrisk->detect(img, keypoints);
    //t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
...
void detKeypointsORB(vector<cv::KeyPoint>& keypoints, cv::Mat& img, bool bVis)
{
    ...
    auto detectorORB = cv::ORB::create();
    //double t = (double)cv::getTickCount();
    detectorORB->detect(img, keypoints);
    //t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
...
void detKeypointsAKAZE(vector<cv::KeyPoint>& keypoints, cv::Mat& img, bool bVis)
{
    ...
    auto detectorAkaze = cv::AKAZE::create();
    //double t = (double)cv::getTickCount();
...
void detKeypointsSIFT(vector<cv::KeyPoint>& keypoints, cv::Mat& img, bool bVis)
{
    ...
    auto detectorSift = cv::xfeatures2d::SIFT::create();
    //double t = (double)cv::getTickCount();
